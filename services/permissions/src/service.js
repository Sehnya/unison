...
import { computePermissions } from './compute.js';
import { hasPermission } from './bitset.js';
import { RoleNotFoundError, CannotModifyEveryoneError, GuildNotFoundError, MemberNotFoundError, ChannelNotFoundError, RoleAlreadyAssignedError, RoleNotAssignedError, } from './errors.js';
/**
 * Permissions Service
 */
export class PermissionsService {
     ...
     * Requirements: 6.1
     */
    async createRole(guildId, name, permissions, color) {
        // Verify guild exists
     ...
     * Requirements: 6.3
     */
    async reorderRoles(guildId, rolePositions) {
        // Verify guild exists
        const guild = await this.repository.getGuildById(guildId);
        if (!guild) {
            throw new GuildNotFoundError();
     ...
     * Requirements: 8.5
     */
    async setChannelOverwrite(channelId, targetId, targetType, allow, deny) {
     ...
     * Requirements: 8.5
     */
    async deleteChannelOverwrite(channelId, targetId) {
     ...
     * Requirements: 7.1, 7.2
     */
    async computePermissions(userId, channelId) {
        // Get channel's guild
     ...
     */
    async hasPermission(userId, channelId, permission) {
