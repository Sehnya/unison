...
export class GuildService {
  ...
  async createGuild(
    ...
    try {
      ...
      // Requirements: 3.3
      const everyoneRole = await this.repository.createRole(
        guildId, // @everyone role ID = guild ID
        guildId,
        '@everyone',
      ...
      const defaultChannel = await this.repository.createChannel(
        channelId,
        guildId,
        'general',
        ChannelType.TEXT,
      ...
      // Add creator as first member with owner status
      await this.repository.addMember(guildId, ownerId, true, client);
  ...
  async updateGuild(
    ...
    updates: { name?: string; description?: string | null; icon?: string | null; banner?: string | null }
  ): Promise<Guild> {
    // Verify guild exists
    const guild = await this.repository.getGuildById(guildId);
    if (!guild) {
   ...
   * Requirements: 3.6
   */
  async deleteGuild(guildId: Snowflake, requesterId: Snowflake): Promise<void> {
    // Verify guild exists
    const guild = await this.repository.getGuildById(guildId);
    if (!guild) {
      throw new GuildNotFoundError();
   ...
   */
  async joinGuild(userId: Snowflake, inviteCode: string): Promise<Member> {
    // Validate invite
   ...
   * Requirements: 4.3
   */
  async leaveGuild(guildId: Snowflake, userId: Snowflake): Promise<void> {
    // Verify guild exists
    const guild = await this.repository.getGuildById(guildId);
    if (!guild) {
      throw new GuildNotFoundError();
   ...
   */
  async unbanMember(guildId: Snowflake, targetUserId: Snowflake): Promise<void> {
    // Verify guild exists
    const guild = await this.repository.getGuildById(guildId);
    if (!guild) {
      throw new GuildNotFoundError();
   ...
   * Get a member
   */
  async getMember(guildId: Snowflake, userId: Snowflake): Promise<Member> {
   ...
   * Requirements: 5.5
   */
  async revokeInvite(guildId: Snowflake, code: string): Promise<void> {
