...
import { MessageNotFoundError, ChannelNotFoundError, MissingPermissionError, NotMessageAuthorError, MessageTooLongError, EmptyMessageError, MessageDeletedError, } from './errors.js';
/**
...
export class MessagingService {
     ...
     */
    async createMessage(channelId, authorId, content) {
        // Validate content
        ...
        const canSend = await this.permissionChecker.hasPermission(authorId, channelId, Permissions.SEND_MESSAGES);
        if (!canSend) {
        ...
        // Create the message
        // Requirements: 9.2
        const message = await this.repository.createMessage(messageId, channelId, authorId, trimmedContent, validMentions, validRoleMentions, createdAt);
    ...
    async getMessages(channelId, userId, options = {}) {
        ...
        const canRead = await this.permissionChecker.hasPermission(userId, channelId, Permissions.READ_MESSAGE_HISTORY);
        if (!canRead) {
    ...
    async updateMessage(messageId, userId, content) {
        ...
        if (!isAuthor) {
            const canManage = await this.permissionChecker.hasPermission(userId, message.channel_id, Permissions.MANAGE_MESSAGES);
            if (!canManage) {
