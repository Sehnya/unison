<script lang="ts">
  ...
  export let unreadDMCount: number = 0;
...
<aside class="sidebar">
  ...
  <div class="section">
    ...
    <button 
      class="nav-btn" 
      class:active={selectedSection === 'main'}
      on:click={() => dispatch('selectSection', { section: 'main' })}
    ...
    <button 
      ...
      on:click={() => dispatch('selectSection', { section: 'settings' })}
      aria-label="Settings"
    >
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <button 
      class="nav-btn inbox-btn"
      class:active={selectedSection === 'inbox'}
      on:click={() => dispatch('openInbox')}
      aria-label="Messages"
    >
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {#if unreadDMCount > 0}
        <span class="unread-badge">{unreadDMCount > 99 ? '99+' : unreadDMCount}</span>
      {/if}
    </button>
  </div>

  <!-- Guilds Section -->
  <div class="section guilds-section">
    <span class="section-label">GUILDS</span>
    
    {#if guilds.length === 0}
      <div class="no-guilds">
        <span class="no-guilds-text">No guilds yet</span>
      </div>
    {:else}
      {#each guilds as guild (guild.id)}
        ...
        >
          ...
          {:else}
            <div class="guild-initials" style="background: {getGuildColor(guild.id)}">
  ...
  <!-- Add Guild Button -->
  <button class="add-btn" on:click={() => dispatch('createGuild')} aria-label="Create or join guild" title="Create or join guild">
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
